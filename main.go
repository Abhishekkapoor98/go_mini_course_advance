...
func ProcessFleet(t []Truck) error {
	...
	// creating the error channel to capture errors from goroutines.
	// to avoid the deadlock, we need to make sure that the channel is buffered
	// or that we close it after all goroutines are done.
	errorChan := make(chan error, len(t))

	// once we open a channel, we need to close it when we're done.
	// We'll use a goroutine to wait for all the trucks to be processed and
	...
	for _, truck := range t {
		...
		go func(truck Truck) {
			if err := processTrucks([]Truck{truck}); err != nil {
				...
				// appending the error channel to capture errors from goroutines
				errorChan <- err
			}
			wg.Done()
		}(truck)
	...
	// Now finally we can listen to the error channel
	// and return any errors that occurred during processing.
	select {
	case err := <-errorChan:
